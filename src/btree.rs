...
// ---------------------------------------------------------------------------
// BTreeInt — integer-keyed B+ tree (u64 keys)
// ---------------------------------------------------------------------------

/// Branch entry size for integer B+ tree: 4 (child) + 8 (key) = 12 bytes.
const INT_BRANCH_ENTRY_SIZE: usize = 12;

/// A B+ tree with u64 keys, for fast rowid-based lookups.
///
/// Leaf page layout:
///   [header: 16 bytes]  (same Page header: magic, flags, num_rows, free_space_offset, next_leaf, prev_leaf)
///   [entry_0][entry_1]...[entry_N]
///   each entry: [key: u64 (8 bytes)][data_len: u16 (2 bytes)][data: data_len bytes]
///   Entries are packed sequentially starting at PAGE_HEADER_SIZE.
///   free_space_offset tracks the end of the last entry.
///
/// Branch page layout:
///   [header: 16 bytes]
///   [child_0: u32][key_0: u64][child_1: u32][key_1: u64]...[child_N: u32]
///   Packed as: child(4) + key(8) repeated, then one trailing child(4).
pub struct BTreeInt<'a> {
    file: &'a mut PageFile,
    root: u32,
}

enum IntInsertResult {
    Fit,
...
impl<'a> BTreeInt<'a> {
    ...
    /// Create a new empty integer B+ tree (single empty leaf page).
    pub fn create(file: &mut PageFile) -> Result<u32> {
        let page_no = file.allocate_page()?;
        let page = Page::new_leaf();
        file.put_page(page_no, page);
    ...
    /// Insert a key-value pair. Returns the (possibly new) root page number.
    pub fn insert(&mut self, key: u64, data: &[u8]) -> Result<u32> {
        let result = self.int_insert_recursive(self.root, key, data)?;
        match result {
            IntInsertResult::Fit => Ok(self.root),
            IntInsertResult::Split { new_page, separator } => {
                let new_root = self.file.allocate_page()?;
                let mut root_page = Page::new_branch();
                int_write_branch_entry(&mut root_page, 0, self.root, separator);
                int_set_branch_child(&mut root_page, 1, new_page);
                root_page.set_num_rows(1);
                root_page.update_checksum();
                self.file.put_page(new_root, root_page);
                self.root = new_root;
    ...
    /// Search for a value by u64 key. Returns the raw data bytes if found.
    pub fn search(&mut self, key: u64) -> Result<Option<Vec<u8>>> {
        self.int_search_recursive(self.root, key)
    }

    /// Delete a key. Returns true if the key existed.
    pub fn delete(&mut self, key: u64) -> Result<bool> {
        self.int_delete_recursive(self.root, key)
    }

    // --- Internal methods ---

    fn int_insert_recursive(
        &mut self,
        page_no: u32,
        key: u64,
        data: &[u8],
    ) -> Result<IntInsertResult> {
        let page = self.file.read_page(page_no)?.clone();

        if page.is_leaf() {
            self.int_insert_into_leaf(page_no, &page, key, data)
        } else {
            let (child_idx, child_page_no) = int_find_child(&page, key);
            let result = self.int_insert_recursive(child_page_no, key, data)?;

            match result {
                IntInsertResult::Fit => Ok(IntInsertResult::Fit),
                IntInsertResult::Split { new_page, separator } => {
                    self.int_insert_into_branch(page_no, child_idx, separator, new_page)
                }
            }
        }
    }

    fn int_insert_into_leaf(
        &mut self,
        page_no: u32,
        page: &Page,
        key: u64,
        ...
        // Binary search for insertion point and duplicate check.
        let (pos, found) = int_find_insertion_point(page, key);
        if found {
            return Err(BoogyError::DuplicateKey(format!("rowid:{key}")));
        }

        // Entry size: 8 (key) + 2 (data_len) + data.len()
        let entry_size = 8 + 2 + data.len();

        // Check if the new entry fits.
        // Current data occupies PAGE_HEADER_SIZE..free_space_offset.
        let current_used = page.free_space_offset() as usize;
        let needed = current_used + entry_size + CHECKSUM_SIZE;

        if needed <= PAGE_SIZE {
            // Fits: rebuild leaf with new entry inserted at pos.
            let snapshot = page.data;
            let page = self.file.write_page(page_no)?;
            int_write_leaf_with_insert(page, &snapshot, num_entries, pos, key, data);
            page.update_checksum();
            Ok(IntInsertResult::Fit)
        } else {
            // Split.
            let snapshot = page.data;
            let next_leaf = page.next_leaf();
            ...
            // Extract separator key from the first entry in the right half.
            let separator = int_key_at_virtual_pos(&snapshot, num_entries, pos, key, mid);

            // Write left half (indices 0..mid).
            let left_page = self.file.write_page(page_no)?;
            int_write_leaf_range(left_page, &snapshot, num_entries, pos, key, data, 0, mid);
            left_page.set_next_leaf(new_page_no);
            left_page.set_prev_leaf(prev_leaf);
            left_page.update_checksum();
            ...
            // Write right half (indices mid..total).
            let mut right_page = Page::new_leaf();
            int_write_leaf_range(
                &mut right_page, &snapshot, num_entries, pos, key, data, mid, total,
            );
            right_page.set_prev_leaf(page_no);
            right_page.set_next_leaf(next_leaf);
            right_page.update_checksum();
    ...
    fn int_insert_into_branch(
        ...
        let max_keys =
            (PAGE_SIZE - PAGE_HEADER_SIZE - 4 - CHECKSUM_SIZE) / INT_BRANCH_ENTRY_SIZE;

        if num_keys < max_keys {
            let page = self.file.write_page(page_no)?;
            int_insert_branch_entry(page, child_idx, separator, new_child);
            page.update_checksum();
            Ok(IntInsertResult::Fit)
        } else {
            let (children, keys) = int_collect_branch_flat(&page);
            let mut new_children = children;
            let mut new_keys = keys;
            ...
            let left_page = self.file.write_page(page_no)?;
            int_rebuild_branch_flat(left_page, left_children, left_keys);
            left_page.update_checksum();

            let new_page_no = self.file.allocate_page()?;
            let mut new_page = Page::new_branch();
            int_rebuild_branch_flat(&mut new_page, right_children, right_keys);
            new_page.update_checksum();
            self.file.put_page(new_page_no, new_page);
    ...
    fn int_search_recursive(&mut self, page_no: u32, key: u64) -> Result<Option<Vec<u8>>> {
        let page = self.file.read_page(page_no)?.clone();
        ...
        if page.is_leaf() {
            ...
            let (pos, found) = int_find_insertion_point(&page, key);
            if found {
                let (_, data) = int_read_entry(&page.data, pos);
                return Ok(Some(data.to_vec()));
            }
            Ok(None)
        } else {
            let (_, child_page_no) = int_find_child(&page, key);
            self.int_search_recursive(child_page_no, key)
        }
    }

    fn int_delete_recursive(&mut self, page_no: u32, key: u64) -> Result<bool> {
        let page = self.file.read_page(page_no)?.clone();
        ...
        if page.is_leaf() {
            ...
            let page = self.file.write_page(page_no)?;
            int_write_leaf_without(page, &snapshot, num_entries, pos);
            page.set_next_leaf(next_leaf);
            page.set_prev_leaf(prev_leaf);
            page.update_checksum();
            Ok(true)
            ...
            let (_, child_page_no) = int_find_child(&page, key);
            self.int_delete_recursive(child_page_no, key)
        }
    }
}

// ---------------------------------------------------------------------------
// BTreeInt leaf page helpers
// ---------------------------------------------------------------------------

/// Compute the byte offset where entry `i` starts in an int-leaf page.
/// Entries are packed sequentially: [key:8][data_len:2][data:data_len]
fn int_entry_offset(data: &[u8; PAGE_SIZE], i: usize) -> usize {
    let mut offset = PAGE_HEADER_SIZE;
    for _ in 0..i {
        // skip key (8) + read data_len (2) + skip data
        let dlen = u16::from_le_bytes([data[offset + 8], data[offset + 9]]) as usize;
        offset += 8 + 2 + dlen;
    }
    offset
}

/// Read the key and data slice for entry `i`.
fn int_read_entry<'d>(data: &'d [u8; PAGE_SIZE], i: usize) -> (u64, &'d [u8]) {
    let offset = int_entry_offset(data, i);
    let key = u64::from_le_bytes(data[offset..offset + 8].try_into().unwrap());
    let dlen = u16::from_le_bytes([data[offset + 8], data[offset + 9]]) as usize;
    let data_start = offset + 10;
    (key, &data[data_start..data_start + dlen])
}

/// Read just the key for entry `i`.
fn int_read_key(data: &[u8; PAGE_SIZE], i: usize) -> u64 {
    let offset = int_entry_offset(data, i);
    u64::from_le_bytes(data[offset..offset + 8].try_into().unwrap())
}

/// Binary search within an int-leaf page.
/// Returns (index, found).
fn int_find_insertion_point(page: &Page, key: u64) -> (usize, bool) {
    let num_entries = page.num_rows() as usize;
    if num_entries == 0 {
        return (0, false);
    }
    ...
    while lo < hi {
        let mid = lo + (hi - lo) / 2;
        let mid_key = int_read_key(&page.data, mid);
...
/// Write a leaf page with a new entry inserted at `insert_pos`.
fn int_write_leaf_with_insert(
    page: &mut Page,
    snapshot: &[u8; PAGE_SIZE],
    old_count: usize,
    insert_pos: usize,
    ...
    let mut write_pos = PAGE_HEADER_SIZE;
    let mut src_idx = 0usize;
    let mut dst_idx = 0usize;
    ...
    while dst_idx < total {
        if dst_idx == insert_pos {
            // Write new entry.
            page.data[write_pos..write_pos + 8].copy_from_slice(&new_key.to_le_bytes());
            page.data[write_pos + 8..write_pos + 10]
                .copy_from_slice(&(new_data.len() as u16).to_le_bytes());
            page.data[write_pos + 10..write_pos + 10 + new_data.len()]
                .copy_from_slice(new_data);
            write_pos += 10 + new_data.len();
            dst_idx += 1;
        } else {
            // Copy existing entry from snapshot.
            let off = int_entry_offset(snapshot, src_idx);
            let dlen =
                u16::from_le_bytes([snapshot[off + 8], snapshot[off + 9]]) as usize;
            let entry_size = 10 + dlen;
            page.data[write_pos..write_pos + entry_size]
                .copy_from_slice(&snapshot[off..off + entry_size]);
...
fn int_write_leaf_without(
    ...
    for src_idx in 0..old_count {
        ...
        let off = int_entry_offset(snapshot, src_idx);
        let dlen = u16::from_le_bytes([snapshot[off + 8], snapshot[off + 9]]) as usize;
        let entry_size = 10 + dlen;
        page.data[write_pos..write_pos + entry_size]
            .copy_from_slice(&snapshot[off..off + entry_size]);
        write_pos += entry_size;
...
/// Write a range [range_start..range_end) of the virtual entry sequence
/// (old entries + new entry at insert_pos) into the page.
fn int_write_leaf_range(
    page: &mut Page,
    snapshot: &[u8; PAGE_SIZE],
    _old_count: usize,
    insert_pos: usize,
    ...
    for dst_idx in 0..count {
        ...
        if virtual_idx == insert_pos {
            ...
            let off = int_entry_offset(snapshot, orig_idx);
            let dlen =
                u16::from_le_bytes([snapshot[off + 8], snapshot[off + 9]]) as usize;
            let entry_size = 10 + dlen;
            page.data[write_pos..write_pos + entry_size]
...
/// Get the key at a virtual position (old entries + new entry at insert_pos).
fn int_key_at_virtual_pos(
    snapshot: &[u8; PAGE_SIZE],
    old_count: usize,
    insert_pos: usize,
    new_key: u64,
    ...
    if virtual_pos == insert_pos {
        ...
        int_read_key(snapshot, orig_idx)
    }
}

// ---------------------------------------------------------------------------
// BTreeInt branch page helpers
// ---------------------------------------------------------------------------

fn int_get_branch_child(page: &Page, idx: usize) -> u32 {
    let offset = PAGE_HEADER_SIZE + idx * INT_BRANCH_ENTRY_SIZE;
    u32::from_le_bytes(page.data[offset..offset + 4].try_into().unwrap())
}

fn int_get_branch_key(page: &Page, idx: usize) -> u64 {
    let offset = PAGE_HEADER_SIZE + idx * INT_BRANCH_ENTRY_SIZE + 4;
    u64::from_le_bytes(page.data[offset..offset + 8].try_into().unwrap())
}

fn int_find_child(page: &Page, key: u64) -> (usize, u32) {
    let num_keys = page.num_rows() as usize;
    for i in 0..num_keys {
        let k = int_get_branch_key(page, i);
        if key < k {
            return (i, int_get_branch_child(page, i));
        }
    }
    let last_child_offset = PAGE_HEADER_SIZE + num_keys * INT_BRANCH_ENTRY_SIZE;
    let child = u32::from_le_bytes(
        page.data[last_child_offset..last_child_offset + 4]
            .try_into()
            .unwrap(),
...
fn int_write_branch_entry(page: &mut Page, idx: usize, child: u32, key: u64) {
    let offset = PAGE_HEADER_SIZE + idx * INT_BRANCH_ENTRY_SIZE;
    page.data[offset..offset + 4].copy_from_slice(&child.to_le_bytes());
    page.data[offset + 4..offset + 12].copy_from_slice(&key.to_le_bytes());
}

fn int_set_branch_child(page: &mut Page, idx: usize, child: u32) {
    let offset = PAGE_HEADER_SIZE + idx * INT_BRANCH_ENTRY_SIZE;
    page.data[offset..offset + 4].copy_from_slice(&child.to_le_bytes());
}

fn int_insert_branch_entry(page: &mut Page, child_idx: usize, key: u64, new_child: u32) {
    let (children, keys) = int_collect_branch_flat(page);
    let mut new_children = children;
    let mut new_keys = keys;
    new_keys.insert(child_idx, key);
    new_children.insert(child_idx + 1, new_child);
    int_rebuild_branch_flat(page, &new_children, &new_keys);
    page.set_num_rows(new_keys.len() as u16);
}

fn int_collect_branch_flat(page: &Page) -> (Vec<u32>, Vec<u64>) {
    let num_keys = page.num_rows() as usize;
    let mut children = Vec::with_capacity(num_keys + 1);
    let mut keys = Vec::with_capacity(num_keys);
    for i in 0..num_keys {
        children.push(int_get_branch_child(page, i));
        keys.push(int_get_branch_key(page, i));
    }
    children.push(int_get_branch_child(page, num_keys));
    (children, keys)
}

fn int_rebuild_branch_flat(page: &mut Page, children: &[u32], keys: &[u64]) {
    page.set_flags(PAGE_BRANCH);
    page.set_num_rows(keys.len() as u16);
    page.data[PAGE_HEADER_SIZE..PAGE_SIZE - CHECKSUM_SIZE].fill(0);
    for (i, key) in keys.iter().enumerate() {
        int_write_branch_entry(page, i, children[i], *key);
    }
    int_set_branch_child(page, keys.len(), children[keys.len()]);
}
...
mod tests {
    ...
    #[test]
    fn test_int_insert_and_search() {
        let tmp = NamedTempFile::new().unwrap();
        let mut pf = PageFile::open(tmp.path()).unwrap();
        let root = BTreeInt::create(&mut pf).unwrap();
        let mut tree = BTreeInt::new(&mut pf, root);

        let data = b"hello world";
        tree.insert(42, data).unwrap();
    ...
    #[test]
    fn test_int_duplicate_key_rejected() {
        let tmp = NamedTempFile::new().unwrap();
        let mut pf = PageFile::open(tmp.path()).unwrap();
        let root = BTreeInt::create(&mut pf).unwrap();
        let mut tree = BTreeInt::new(&mut pf, root);

        tree.insert(1, b"data1").unwrap();
        assert!(tree.insert(1, b"data2").is_err());
    }
    ...
    #[test]
    fn test_int_many_inserts_trigger_split() {
        let tmp = NamedTempFile::new().unwrap();
        let mut pf = PageFile::open(tmp.path()).unwrap();
        let mut root = BTreeInt::create(&mut pf).unwrap();

        for i in 0..200u64 {
            let data = format!("value_{i}");
            let mut tree = BTreeInt::new(&mut pf, root);
            root = tree.insert(i, data.as_bytes()).unwrap();
        }
    ...
    #[test]
    fn test_int_500_inserts_with_row_data() {
        let tmp = NamedTempFile::new().unwrap();
        let mut pf = PageFile::open(tmp.path()).unwrap();
        let mut root = BTreeInt::create(&mut pf).unwrap();

        for i in 0..500u64 {
            let row = row::encode_row(&format!("id_{i}"), &[(0, &Value::Integer(i as i64))]);
            let mut tree = BTreeInt::new(&mut pf, root);
        ...
        for i in 0..500u64 {
            let mut tree = BTreeInt::new(&mut pf, root);
            let result = tree.search(i).unwrap();
            assert!(result.is_some(), "missing rowid {i}");
            let decoded = row::decode_row(&result.unwrap()).unwrap();
            assert_eq!(decoded.id, format!("id_{i}"));
        }
    }
}
